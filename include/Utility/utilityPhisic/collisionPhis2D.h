...
namespace phis2D
{
	...
	public: static void ProjectCircle(collider::CircleCollider* circle, const v2f& axis, float& min, float& max)// точно норм
	{
	...
	private: static v2f FindArithmeticMean(const std::pair<const v2f* const&, size_t>& vertices)//точно норм
	{
		v2f sum;
		for (int i = 0; i < vertices.second; i++)
		{
	...
	private: static bool IntersectCircles(collider::CircleCollider* circleA, collider::CircleCollider* circleB, v2f& normal, float& depth)//точнр норм
		{
			normal = v2f();
			depth = 0.f;
	...
	private: static bool IntersectPolygons(collider::VirtualCollider* colliderA, collider::VirtualCollider* colliderB, v2f& normal, float& depth)//точно норм
		{
